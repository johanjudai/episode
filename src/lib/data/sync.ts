...
export async function syncSeriesFull(
  ...
  if (opts.follow) {
    ...
    const existing = await getSeries(db, tmdbId);
    if (!existing) {
      await followSeriesMutation(db, {
        tmdbId: detail.id,
        name: detail.name,
  ...
  await Promise.all(
    (detail.seasons ?? [])
      ...
      .map((s) => syncSeason(db, apiKey, tmdbId, s.season_number).catch(() => undefined))
  );
}
...
export async function syncSeason(
  ...
  if (await seasonExists(db, seriesTmdbId, seasonNumber)) return;
  const tmdb = createTmdbClient({ apiKey });
